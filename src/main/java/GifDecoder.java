...
@SuppressWarnings({"PMD.DataflowAnomalyAnalysis", "PMD.NullAssignment",
        "PMD.AvoidLiteralsInIfCondition", "PMD.MissingBreakInSwitch"})
public class GifDecoder {
    ...
    private transient int width; // full image width
    private transient int height; // full image height
    private transient boolean gctFlag; // global color table used
    private transient int gctSize; // size of global color table
    private transient int loopCount = 1; // iterations; 0 = repeat forever
    private transient int[] gct; // global color table
    private transient int[] lct; // local color table
    private transient int[] act; // active color table
    private transient int bgIndex; // background color index
    private transient int bgColor; // background color
    private transient int lastBgColor; // previous bg color
    private transient int pixelAspect; // pixel aspect ratio
    private transient boolean lctFlag; // local color table flag
    private transient boolean interlace; // interlace flag
    private transient int lctSize; // local color table size
    private transient int ix;
    private transient int iy;
    private transient int iw;
    private transient int ih; // current image rectangle
    private transient int lrx;
    private transient int lry;
    private transient int lrw;
    private transient int lrh;
    private transient DixieMap image; // current frame
    private transient DixieMap lastPixmap; // previous frame
    private transient byte[] block = new byte[256]; // current data block
    private transient int blockSize = 0; // block size last graphic control extension info
    private transient int dispose = 0; // 0;1;2;3 no;leave in place;restore to bg;restore to prev
    private transient int lastDispose = 0;
    private transient boolean transparency = false; // use transparent color
    private transient int delay = 0; // delay in milliseconds
    private transient int transIndex; // transparent color index
    ...
    private transient Vector<GifFrame> frames; // frames read from current file
        ...
        DixieMap(int[] data, int w, int h, Pixmap.Format f) {
            ...
            for (y = 0; y < h; y++) {
                for (x = 0; x < w; x++) {
                    ...
                    int pxl_RGBA8888 = ((pxl_ARGB8888 >> 24) & 0x000000ff) | ((pxl_ARGB8888 << 8)
                            & 0xffffff00);
    ...
    /**
     * Creates new frame image from current data (and previous frames
     * as specified by their disposition codes).
     */
